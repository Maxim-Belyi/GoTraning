...
4.  **Создайте .env файл:**
    В корне проекта создайте файл `.env`, в строке подключения вместо `localhost` нужно указать имя сервиса базы данных (`mysql`):
    ```env
    DB_DSN=appuser:apppass@tcp(mysql:3306)/golangBd?parseTime=true
    ```
    *(Если вы решите запускать Go-приложение локально без Docker, замените `mysql` обратно на `localhost`)*

5.  **Запуск проекта :**
    Проект полностью контейнеризирован и использует утилиту **Air** для горячей перезагрузки (live-reload). Чтобы запустить базу данных и само приложение, выполните:
    ```sh
    docker compose up --build
    ```
    После успешной сборки сайт будет доступен в браузере по адресу `http://localhost:8080`.
  

**Альтернативный вариант: Локальный запуск приложения**
Если вам нужно запустить само Go-приложение без Docker, поднимите только базу данных:
